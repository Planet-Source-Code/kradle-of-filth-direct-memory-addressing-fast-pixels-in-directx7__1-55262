...
Public Sub Game_Loop()
    ...
    'This draws lines instantly using direct memory addressing. Problem
    'is that I'm using (functions/subs) within (functions/subs) within
    '(functions/subs), which bogs it down but it was easier for me to
    'work with that way. Optimizations are possible to speed it up even
    'more.
    
    '60 FPS for 200 lines per frame (320x240)!!!
    ...
    For Y = 0 To 200
        ...
        Clip_X1 = Rnd * (Window_Width - 1)
        Clip_Y1 = Rnd * (Window_Height - 1)
        Clip_X2 = Rnd * (Window_Width - 1)
        Clip_Y2 = Rnd * (Window_Height - 1)
        
        If Clip_Line(Clip_X1, Clip_Y1, Clip_X2, Clip_Y2) = 1 Then
            
            Draw_Line_Bresenham Clip_X1, Clip_Y1, Clip_X2, Clip_Y2, RGB(Rnd * 255, 0, 255)
